...
$strSQLcase = "SELECT COUNT(*) CS FROM numbetall";
...
$strSQLcasecoun = "SELECT status, COUNT(status) AS countall FROM numbetall WHERE status='1';";
$objQuerycasecoun = mysqli_query($objCon, $strSQLcasecoun);

$rowcasecoun = mysqli_num_rows($objQuerycasecoun);

$strSQLcasecounnumbet = "SELECT status, COUNT(status) AS countalll FROM numbetall WHERE status='0';";
$objQuerycasecounnumbet  = mysqli_query($objCon, $strSQLcasecounnumbet);

$rowcasecounnumbet = mysqli_num_rows($objQuerycasecounnumbet);


$strSQLcasecounnumbetsell = "SELECT * from numbetall WHERE status='0';";
$objQuerycasecounnumbetsell  = mysqli_query($objCon, $strSQLcasecounnumbetsell);

$rowcasecounnumbetsell = mysqli_num_rows($objQuerycasecounnumbetsell);
...
<center>  <h2>  ตรวจสอบลอตเตอรี่คงเหลือ </h2></center>
...
echo "<h5>  จำนวนทั้งหมด :" .$f["CS"] .  "ใบ</h5>";
}
?>
...
while( $u = mysqli_fetch_assoc($objQuerycasecoun)) {

echo "<h5>ขายไป :" .$u["countall"] .  "ใบ</h5>";
}
...
while( $ssa = mysqli_fetch_assoc($objQuerycasecounnumbet)) {

echo "<h5>เหลือ :" .$ssa["countalll"] .  "ใบ</h5>";
}
...
<?php
 ...
 echo  "<DIV id=tbl-container>";
     echo  "<TABLE BORDER=1 WIDTH=246 id=tbl>";
     echo "<THEAD>";
          echo  "<tr>";
echo "<TH width=150>เลขที่เหลือ</TH>";
 echo  "</tr>";
echo "</THEAD>";
...
while( $P = mysqli_fetch_assoc($objQuerycasecounnumbetsell)) {

 echo  "<tr>";

    echo "<td><h4>" .$P["numbet_id"].  "</h4></td> ";
